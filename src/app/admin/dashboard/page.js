  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm mt-1">Welcome back! Manage your portfolio content.</p>
      </div>

      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-40 bg-[#111118] border border-[#1e1e2e] rounded-xl animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {cards.map((card) => {
            ...
            return (
              <Link
                key={card.title}
                href={card.href}
                className={`group bg-[#111118] border border-[#1e1e2e] rounded-xl p-6 hover:border-[#2a2a3a] transition-all duration-300 hover:shadow-lg hover:shadow-${card.color}-500/5`}
              >
      ...
      <div className="mt-8">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            href="/admin/resume"
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#1a1a2a] hover:bg-[#222233] text-gray-300 text-sm font-medium rounded-lg border border-[#2a2a3a] transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
