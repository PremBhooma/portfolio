...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 z-50 h-full w-64 bg-[#111118] border-r border-[#1e1e2e] transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between h-16 px-6 border-b border-[#1e1e2e]">
          <Link href="/admin/dashboard" className="text-lg font-bold bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">
            AstroDev Admin
          </Link>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-[#1e1e2e]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-400 truncate max-w-[120px]">{admin?.email}</span>
            </div>
      ...
      <div className="lg:ml-64">
        {/* Top bar */}
        <header className="sticky top-0 z-30 h-16 bg-[#0a0a0f]/80 backdrop-blur-xl border-b border-[#1e1e2e] flex items-center px-6">
